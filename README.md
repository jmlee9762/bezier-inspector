...
Visualize anchor points, Bézier handles, connector lines, and typography metrics directly on the Illustrator artboard. 
앵커 포인트, 베지어 핸들, 연결선, 그리고 타이포그래피 메트릭을 Illustrator 아트보드 위에서 실시간으로 시각화하는 CEP 플러그인입니다.
...
Restart Adobe Illustrator and open:
```txt
Window → Extensions → Bézier Inspector
```

Illustrator를 재실행한 후 `Window → Extensions → Bézier Inspector`에서 실행하세요.
